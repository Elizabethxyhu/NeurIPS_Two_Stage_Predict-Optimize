...
#                for k in range(rowSizeG):
#                    for q in range(n_var):
#                        if q == j:
#                            fGx[j][k][q] = mu / np.square(h_numpy[k] - np.dot(G_numpy[k], x_numpy)) * G_numpy[k][j] * x_numpy[q] + mu / (h_numpy[k] - np.dot(G_numpy[k], x_numpy))
#                        else:
#                            fGx[j][k][q] = mu / np.square(h_numpy[k] - np.dot(G_numpy[k], x_numpy)) * G_numpy[k][j] * x_numpy[q]
            for j in range(n_var):
                matTemp = mu * x_numpy[j] * G_numpy.T.dot(hGx2)
                matTemp[j,:] = matTemp[j,:] + mu / hGx
                fGx[j] = matTemp.T
#            print(fGx.shape)   # (48, 49, 48)
...
#            print(dRxs2.shape, dxs2xs1.shape, dxG.shape, dRxs1.shape)   # (10,) (10, 10) (10, 2, 10) (10,)
#            dRxs2 = dRxs2 * del_x.numpy()
            Grad1 = dRxs2.dot(dxs2xs1)
            for i in range(n_var):
                dxG[..., i] = np.dot(Grad1, dxG[..., i]) + np.dot(dRxs1, dxG[..., i])
            
#            violateFactor = violateFactor.numpy()
#            violateEdgeIndex = violateEdgeIndex.numpy()
##            print(violateFactor, violateEdgeIndex)
#            if violateFactor > 1:
#                violateG = G_numpy[violateEdgeIndex]
#                constantItem = np.dot(violateG, x_sol/violateFactor)
#                constantItem = violateFactor/constantItem
#                corrGradTemp = np.zeros((n_var, n_var))
#                for i in range(n_var):
#                    for j in range(n_var):
#                        corrGradTemp[i][j] = -x_numpy[i]*violateG[j]
#                #print(violateG.shape, corrGradTemp.shape)
#                E = np.eye(n_var)
#                violateFactorE = E * violateFactor
#                corrGrad = corrGradTemp + violateFactorE
##                print(corrGrad.shape)  # (48, 48)
##                dxh = np.dot(corrGrad, dxh)
#                corrdxG = np.zeros((n_var, rowSizeG, n_var))
#                for i in range(n_var):
#                    corrdxG[..., i] = np.dot(corrGrad, dxG[..., i])
##                dxG = dxG
##                penalty = 1
##                print(penalty)
##                dxG = (1+penalty-penalty/violateFactor)*dxG
##                print(penalty)
#                for i in range(n_var):
#                    dxG[..., i] = np.dot(1+penalty, corrdxG[..., i]) + np.dot(penalty, dxG[..., i])
##                    dxG[..., i] = np.dot(1+penalty-penalty/violateFactor, dxG[..., i])
##                print(violateFactor)
...
#            c_grad[ctx.var_index] = dh
            c_grad = -dG.T
#            print(c_grad)
#            print(c_grad.shape) # torch.Size([4, 10]) i.e. (rowSizeG, colSizeG)
